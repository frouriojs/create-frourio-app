...
import { API_ORIGIN, API_UPLOAD_DIR, API_USER_ID, API_USER_PASS } from './envValues';

const iconsDir = API_UPLOAD_DIR && path.resolve(API_UPLOAD_DIR, 'icons');

const createIconURL = (dir: string, name: string) => `${API_ORIGIN}/${dir}icons/${name}`;

const getUserIconName = () => 'user-icon';
...
export const changeIcon = async (id: string, iconFile: MultipartFile) => {
  ...
  if (!iconsDir) throw new Error('API_UPLOAD_DIR is not configured.');

  await fs.promises.mkdir(iconsDir, { recursive: true });

  await fs.promises.writeFile(path.resolve(iconsDir, iconName), await iconFile.toBuffer());

  return { id, ...getUserInfo() };
};
